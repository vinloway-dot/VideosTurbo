"""POSIX credential storage backend for Thumbnail Prompt."""
...
class _DirectoryHandles:
    ...
    def parent_fd(self) -> int:
        return self.components[-1].parent_fd

    @property
    def directory_fd(self) -> int:
        return self.components[-1].directory_fd
    ...
    def descriptors(self) -> tuple[int, ...]:
        return (self.anchor_fd, *(item.directory_fd for item in self.components))
...
class ThumbnailPromptSettingsService:
    ...
    def list_providers(self) -> list[ThumbnailPromptProviderMetadata]:
        ...
        return [
            self._provider_from_config(
                provider_id, loaded.configured, status=loaded.status
            )
    ...
    def get_provider(self, provider_id: str) -> ThumbnailPromptProviderMetadata:
        ...
        return self._provider_from_config(
            normalized, loaded.configured, status=loaded.status
        )

    def get_settings(self) -> ThumbnailPromptSettings:
        loaded = self._read_loaded_settings()
        return self._settings_from_config(loaded.configured, status=loaded.status)
    ...
    def update_settings(
        ...
        return self._settings_from_config(configured, status="valid")
    ...
    def _locked_directory(self):
        ...
        try:
            ...
            try:
                self._private_regular_file_stat(lock_fd)
                os.fchmod(lock_fd, 0o600)
            except (OSError, _UnsafeSettingsPath) as exc:
                raise _UnsafeSettingsPath("settings lock is unsafe") from exc
            try:
                fcntl.flock(lock_fd, fcntl.LOCK_EX)
            except OSError as exc:
                raise _UnsafeSettingsPath(
                    "settings lock could not be acquired"
                ) from exc
            lock_acquired = True
            ...
            if handles is not None:
                for file_descriptor in reversed(handles.descriptors):
                    try:
        ...
        try:
            ...
            if anchor_fd is not None:
                try:
                    ...
                    pass
            raise _UnsafeSettingsPath("settings anchor is unavailable") from exc
        if not stat.S_ISDIR(anchor_stat.st_mode) or anchor_stat.st_nlink < 2:
            try:
                os.close(anchor_fd)
            except OSError:
                pass
            raise _UnsafeSettingsPath("settings anchor is unsafe")
        ...
        try:
            for index, component in enumerate(components):
                ...
                try:
                    ...
                    if is_leaf:
                        self._require_safe_directory(component_stat)
                    elif (
                        not stat.S_ISDIR(component_stat.st_mode)
                        or component_stat.st_nlink < 2
                    ):
                ...
                if is_leaf:
                    return _DirectoryHandles(
                        ...
                        anchor_owner=anchor_stat.st_uid,
                        anchor_mode=stat.S_IMODE(anchor_stat.st_mode),
                        components=tuple(retained_components),
                    )
            raise _UnsafeSettingsPath("dedicated settings leaf is missing")
        except Exception:
            cleanup_error = None
            for file_descriptor in reversed(
                (anchor_fd, *(item.directory_fd for item in retained_components))
            ):
                try:
                    os.close(file_descriptor)
                except OSError as exc:
    ...
    def _verify_leaf_identity_locked(self, handles: _DirectoryHandles) -> None:
        try:
            anchor = os.fstat(handles.anchor_fd)
        except OSError as exc:
            raise _UnsafeSettingsPath("settings anchor identity changed") from exc
        if (
            not stat.S_ISDIR(anchor.st_mode)
            or anchor.st_nlink < 2
            or (anchor.st_dev, anchor.st_ino) != handles.anchor_identity
            or anchor.st_uid != handles.anchor_owner
            or stat.S_IMODE(anchor.st_mode) != handles.anchor_mode
        ):
            raise _UnsafeSettingsPath("settings anchor identity changed")

        for component in handles.components:
            try:
                opened = os.fstat(component.directory_fd)
                named = os.stat(
                    component.name,
                ...
                raise _UnsafeSettingsPath(
                    "settings directory identity changed"
                ) from exc
            for current in (opened, named):
                if (
                    (current.st_dev, current.st_ino) != component.identity
                    or current.st_uid != component.owner
                    or stat.S_IMODE(current.st_mode) != component.mode
                ):
    ...
    def _provider_from_config(
        ...
        *,
        ...
        return ThumbnailPromptProviderMetadata(
            ...
            configuration_error=self._storage_configuration_error(status),
            storage_state=self._public_storage_state(status),
        )

    def _settings_from_config(
        self, configured: dict[str, object], *, status: str
    ) -> ThumbnailPromptSettings:
        storage_state = self._public_storage_state(status)
        ...
        try:
            ...
            return self._settings_from_config(dict(_DEFAULTS), status="corrupt")

    @staticmethod
    def _public_storage_state(status: str) -> str:
        if status == "unsafe":
            return "unsafe"
        if status == "corrupt":
            return "corrupt"
    ...
    @classmethod
    def _storage_configuration_error(cls, status: str) -> str | None:
        state = cls._public_storage_state(status)
        if state == "unsafe":
            return _UNSAFE_SETTINGS_MESSAGE
        if state == "corrupt":
            return _CORRUPT_SETTINGS_MESSAGE
        return None
